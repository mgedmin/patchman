...
class Command(BaseCommand):
    help = 'Toggle check_dns for matching hosts'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--unset',
            action='store_true',
            help='Unset check_dns (default is to set it)',
        )
    ...
    def handle(self, *args, **options):
        ...
        check_dns = not options['unset']
        toggle = 'Setting' if check_dns else 'Unsetting'
        matching_hosts = get_hosts(host, f'{toggle} check_dns')
        for h in matching_hosts:
            info_message(text=str(h))
            h.check_dns = check_dns
            h.save()
